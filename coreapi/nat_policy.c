...
static void linphone_nat_policy_destroy(LinphoneNatPolicy *policy) {
	...
	if (policy->turn_root_certificate) belle_sip_free(policy->turn_root_certificate);
}
...
LinphoneNatPolicy * linphone_config_create_nat_policy_from_section(const LinphoneConfig *config, const char* section) {
	...
	const char *turn_root_certificate = lp_config_get_string(config, section, "turn_root_certificate", NULL);
	...
	if (turn_root_certificate) linphone_nat_policy_set_turn_root_certificate(policy, turn_root_certificate);
	return policy;
}
...
const char * linphone_nat_policy_get_turn_root_certificate(const LinphoneNatPolicy *policy) {
	return policy->turn_root_certificate;
}

void linphone_nat_policy_set_turn_root_certificate(LinphoneNatPolicy *policy, const char *turn_root_certificate) {
	if (turn_root_certificate != NULL) {
		if (policy->turn_root_certificate) belle_sip_free(policy->turn_root_certificate);
		policy->turn_root_certificate = belle_sip_strdup(turn_root_certificate);
	}
}
